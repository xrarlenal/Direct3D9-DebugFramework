...
轻量级、可扩展的 Direct3D9 应用调试框架，聚焦于 D3D9 渲染链路的实时捕获与调试，专为逆向工程、游戏图形调试、D3D9 应用定制化分析场景设计，核心依托即时 GUI 与 API 挂钩技术实现调试.
...
新增 HOOK：基于 MinHook 封装的统一挂钩接口（src/d3d9\_hook.cpp），注册目标 D3D9 函数即可；

定制 UI 面板：继承框架基础 UI 类（src/imgui\_panel\_base.h），实现自定义渲染逻辑；
